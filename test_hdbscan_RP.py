...
print('MPF with RP partition')
"""
Multiple particle filter in RP partition
...
for k in range(Ns): 
    ...
    for i in range(N):
        ...
        subdims = np.array_split(dim, Nb)  # 安全分割，自动处理余数
        block_index = np.zeros(dim_x)
        for m, subdim in enumerate(subdims):
            block_index[subdim] = m  # 直接分配标签
        ...
        Block_mpf2[k, :, i] =  block_index 
        
        
        mpf2.prediction(block_index)
        # blocking step
        mpf2.correction(Y[:, i])
        # Estimation
...
# 打印 MSE
print('MSE for MPF with RP partition:', dict_mpf2['3'])
...
print("RP 分区: bias^2 = {:.6f}, variance = {:.6f}".format(bias2_mpf2, var_mpf2))