# -*- coding: utf-8 -*-
"""
Created on Mon Jan  6 15:07:44 2025
...
class MultipleParticleFilter:
    ...
    def prediction(self, Index_block):
        ...
        for i in range(self.Nb):
            ...
            for j in range(self.Np):
                x = deepcopy(self.x_est)
                x[CurrentIndex] = self.particles[CurrentIndex, j]
                CurrentMean = rungekutta4(x, self.dt, self.F)
                self.particles[CurrentIndex,j] = CurrentMean[CurrentIndex] + multivariate_normal(zeros(len(CurrentIndex)), (self.Q[CurrentIndex])[:,CurrentIndex])
